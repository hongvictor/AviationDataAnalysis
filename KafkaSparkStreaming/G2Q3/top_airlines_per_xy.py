"""
 ...
 This program prints top 10 airlines per origin-destination airports pair 
 Usage: top_airlines_per_xy.py <zk> <topic>
"""
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: top_airlines_per_xy.py <zk> <topic>", file=sys.stderr)
        exit(-1)

    sc = SparkContext(appName="Top airelines per xy")
    ssc = StreamingContext(sc, 10)
    ssc.checkpoint("hdfs://ip-172-31-61-76:9000/checkpoint/G2Q3")
    ...
    kvs = KafkaUtils.createStream(ssc, zkQuorum, "spark-streaming-consumer", {"TopicG2Q3": 12})
    lines = kvs.map(lambda x: x[1])

    # ((origin, dest, airline), ontimeratio)
    counts = lines.map(lambda line: ((line.split("\t")[2], 
        ...
        .map(lambda x: {"origin":x[0][0], 
                        "dest":x[0][1],
                        "airlines":x[1]}) \
        .saveToCassandra("capstone", "top_ten_airlines_per_xy")

#    counts.foreachRDD(println)
